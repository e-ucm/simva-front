			...
			if(status){
				...
				if(status){
					if(status.realtime
						...
						traces = '<span><a onclick="ImsPackagePainter.openTraces(\'' + activity._id + '\',\'' + usernames[i]
							 + '\')">See traces</a></span>';
					}

					if(activity.extra_data.config.backup && results[usernames[i]].backup){
						backup = '<span><a onclick="ImsPackagePainter.downloadBackup(\'' + activity._id + '\',\'' + usernames[i]
							 + '\')">Download</a></span>';
					}
