	...
	paintActivityResult: function(activity, results){
		...
		for (var i = 0; i < usernames.length; i++) {
			...
			if(status){
				...
				result = `<span><a onclick="PainterFactory.Painters["activity"].openResults('${activity.activity_id}','${usernames[i]}')">See Results</a></span>`;
			}


			$(`#result_${activity.activity_id}_${usernames[i]}`).addClass(status ? 'green' : 'red');
