    ...
    router.get('/activitytypes', auth, async (req, res, next) => {
        Simva.getActivityTypes(req.session.id, (error, result) => {
            if(error) {
                ...
                commun['download_tooltip'] = req.t(`download.tooltip`, { ns : 'activities' } );
                commun['test_download_title'] = req.t(`test_download.title`, { ns : 'activities' } );
                commun['test_download_tooltip'] = req.t(`test_download.tooltip`, { ns : 'activities' } );
